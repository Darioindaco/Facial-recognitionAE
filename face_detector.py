...
class FaceDetectorPipeline:
    ...
    def run(self) -> str:
        ...
        outputs = [f"CSV: {output_path}"]

        if self.avid:
            subcap_path = self._write_avid_subcap()
        ...
        if self.render:
            ...
            outputs.append(f"Video: {render_path}")
    ...
    def _write_avid_subcap(self) -> str:
        """Write AVID SubCap file — imports as editable text in AVID timeline."""
        base = os.path.splitext(os.path.basename(self.video_path))[0]
        subcap_path = os.path.join("output", f"{base}_lower_thirds.subcap.txt")

        with open(subcap_path, "w") as f:
            f.write("@ This file was generated by face_detector.py\n")
            f.write(f"@ Source: {self.video_path}\n")
            f.write("\n")
            f.write("<begin subtitles>\n")
            for seg in self.segments:
    ...
    def _write_avid_markers(self) -> str:
        """Write AVID marker import file — places colored markers on timeline."""
        base = os.path.splitext(os.path.basename(self.video_path))[0]
        marker_path = os.path.join("output", f"{base}_lower_thirds.markers.txt")
